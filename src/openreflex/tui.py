...
def statusline(project: Path, *, force_colour: bool = True) -> str:
    ...
    if phase == "reflexing":
        ...
        task = state.get("task") or {}
        if task.get("reflex"):
            label = str(task["reflex"])
            if task.get("reflex_state"):
                label += f" ({task['reflex_state']})"
...
def dashboard(project: Path, *, colour: bool | None = None) -> str:
    ...
    lines += ["", "EXECUTION MEMORY",
              ...
              "", "CURRENT REFLEX",
              f"  name             {task.get('reflex') or 'Project Reflex'}",
              f"  state            {task.get('reflex_state') or state.get('project_reflex_state') or ('learning' if state.get('experiences') else 'warming')}",
              f"  support          {state.get('project_reflex_support', 0)} project experience(s)",
              f"  phase            {phase.upper()}",
